...
flowchart TD
    ...
    H --> I
    I -->|"نواقص مصدر"| C
    I -.->|"خطأ في التعريف: تصحيح متاح قبل الإغلاق"| A
...
### 4.1.1 تصحيح تعريف الدورة

التعريف ليس نهائيًا بمجرد الإنشاء. يقبل `EvaluationCycleService::update` تصحيح خطوات الإنشاء الثلاث ما دامت الدورة في `draft` أو `data_collection`، ويرفض ما بعد ذلك برسالة تسمي الحالة الحالية. لكل حقل حارس بحسب ما يعتمد عليه فعلًا، لا قاعدة واحدة عامة:

| الحقل | القاعدة |
|---|---|
| الاسم والموسم وعدد الأوائل | حر؛ الاسم يبقى غير مكرر داخل المشروع |
| المشروع | لا يعدل؛ تغييره يبطل المقررات والمرشحين ولقطات تسجيلهم، فلا يقبله العقد أصلًا |
| قواعد الاحتساب | تنشأ نسخة سياسة جديدة ويعاد ربط `policy_id` بها؛ لا تعدل سياسة محفوظة في مكانها |
| اسم الفترة | حر دائمًا؛ لا يغير نافذة أي مدخل |
| تواريخ الفترة أو حذفها | يرفض إذا كانت الفترة قد استقبلت حكم مدرس أو تقييم قرآن أو نتيجة امتحان |
| إضافة مقرر | حرة؛ تعاد مزامنة المرشحين بعدها |
| إزالة مقرر | ترفض إذا كان للمقرر مدخلات؛ وإلا حذفت لقطات تسجيله واستبعد من لم يبق له تسجيل |

تطابق الفترات بالتسلسل: التسلسل الموجود يعدل، والجديد ينشأ، والغائب يحذف. يعاد اشتقاق نطاق الدورة من الفترات بعد كل تعديل، كما في الإنشاء تمامًا.

المرشح الذي أزيل مقرره الوحيد يستبعد بـ`status = excluded` وسبب مكتوب، ولا يحذف؛ الاحتساب وفحص الجاهزية يقرآن النشطين فقط، ويبقى أثره في السجل.

يسجل التعديل حدث تدقيق `evaluation.cycle_updated` بالقيم قبل وبعد، شاملة الفترات والمقررات.

### 4.1.2 قوالب قواعد الاحتساب

قواعد الاحتساب قابلة لإعادة الاستخدام بلا جدول جديد: القالب سياسة `evaluation_policies` حالتها `template`، أي سجل قواعد مسمى ومرقّم الإصدار لا يرتبط بأي دورة ولا يلتقطه احتساب.

- `saveTemplate` يحفظ الإعداد المعروض في المحرر باسم يختاره المستخدم. الحفظ باسم قالب موجود ينتج **نسخة أحدث** منه بدل الاصطدام بقيد `name + version` الفريد، فالتحديث والإنشاء إجراء واحد.
- `templates` يعيد أحدث نسخة من كل اسم فقط؛ النسخ الأقدم تاريخ لا خيار في القائمة.
- إنشاء دورة من قالب لا يربطها بسجل القالب: تمر القواعد بـ`createForCycle` فتنشأ للدورة سياسة `approved` مستقلة. تعديل القالب لاحقًا لا يمس أي دورة أنشئت منه.

يعيد `templateFromRules` قواعد أي دورة أو قالب إلى شكل القالب الذي يفهمه محرر القواعد: القيم المحفوظة (`enabled`, `maximum_score`, `rules`, `default_score`) فوق تعريف القالب، مع بقاء `variables` من التعريف لأن السياسة لا تحفظها ولا يعمل المحرر بدونها.
...
|---|---|
| `evaluation_policies` | سياسة JSON ذات اسم وإصدار وحالة واعتماد؛ الاسم والإصدار فريدان. الحالة `approved` لسياسة مرتبطة بدورة، و`template` لقالب قواعد قابل لإعادة الاستخدام لا يرتبط بدورة |
...
- `EvaluationPolicyService`: إنشاء/قراءة السياسة ذات الإصدار، وحفظ قوالب القواعد وقراءة أحدث نسخة من كل قالب. مسار الكتابة واحد (`store`) للسياسة والقالب، ولا يفترق إلا في الاسم والحالة.
- `EvaluationRuleDefinitionService`: تعريف المعايير ومتغيراتها، وتطبيع القواعد المرسلة، وإعادة قواعد محفوظة إلى شكل المحرر (`templateFromRules`).
- `EvaluationCycleService`: إنشاء الدورة، اشتقاق نطاقها من الفترات، منع البدء قبل انتهاء آخر فترة، والتحقق من انتقالات الحالة، وتصحيح التعريف قبل إغلاق جمع البيانات بحوارس حقلية على الفترات والمقررات والقواعد.
...
| `PUT /api/evaluation-cycles/{cycle}` | تصحيح تعريف الدورة قبل إغلاق جمع البيانات؛ لا يقبل `project_id` |
| `GET /api/evaluation-cycles/rule-template` | تعريف المعايير والقواعد للمحرر. مع `?cycle=` يعيده محمّلًا بقواعد دورة قائمة، ومع `?profile=` بقواعد قالب محفوظ |
| `GET /api/evaluation-cycles/rule-profiles` | قوالب القواعد المحفوظة، أحدث نسخة من كل اسم |
| `POST /api/evaluation-cycles/rule-profiles` | حفظ الإعداد المعروض كقالب باسم؛ الاسم الموجود ينتج نسخة أحدث |
...
مسارات القوالب والقالب التعريفي وتصحيح التعريف كلها تحت صلاحية «إدارة دورات التقييم»، ويضيف `PUT /{cycle}` فحص `canManageCycle` على الدورة نفسها.
...
- زر «تعديل التعريف» على بطاقة الدورة، ولا يظهر إلا في `draft` و`data_collection` ولمن يملك «إدارة دورات التقييم».

يعمل معالج الإنشاء نفسه في وضع التعديل بالخطوات والتحقق ذاتها، محمّلًا من الدورة ومن القواعد التي جمّدتها سياستها فعلًا لا من القيم الافتراضية:

- المشروع معطل مع سبب مكتوب تحته.
- التنقل بين الخطوات الثلاث حر، لأن المعروض بيانات محفوظة صحيحة أصلًا.
- أخطاء الحارس الخلفية تعرض على الخطوة التي تملكها: `course_ids` على الأولى و`periods` على الثانية.
- في الخطوة الثالثة بطاقة «القوالب المحفوظة»: اختيار قالب ثم «استبدال القواعد بالقالب» بضغطة صريحة لأنها تستبدل كل المعروض، وحقل اسم مع «حفظ كقالب» لحفظ الإعداد الحالي.

يحيل سجل الدورة إلى هذا المعالج بـ`/evaluations?edit={id}`، وتزال المعلمة عند الإغلاق حتى لا يعيد التحديث فتح النافذة.
...
- شريط حالة واضح.
- زر «تعديل التعريف» بالشرط نفسه، ينقل إلى معالج التعديل في لوحة الدورات.
- مزامنة المرشحين.
...
- تصحيح تعريف الدورة (`evaluation.cycle_updated`)، مع الفترات والمقررات قبل وبعد.
...
- **تعديل تعريف الدورة بعد إغلاق جمع البيانات:** يرفض برسالة تسمي الحالة الحالية.
- **تغيير مشروع الدورة:** لا يقبله عقد التعديل أصلًا.
- **تعديل تواريخ فترة استقبلت مدخلات:** يرفض؛ التسمية وحدها تمر لأنها لا تغير نافذة أي مدخل.
- **حذف فترة استقبلت مدخلات:** يرفض.
- **إزالة مقرر له مدخلات:** يرفض باسم المقرر؛ وإزالة مقرر بلا مدخلات تحذف لقطات تسجيله وتستبعد من لم يبق له تسجيل بدل حذفه.
- **تعديل قواعد دورة:** لا يمس سياسة محفوظة؛ تنشأ نسخة جديدة ويعاد ربط الدورة بها، والتشغيلات السابقة تحتفظ بـ`policy_snapshot`.
- **حفظ قالب باسم موجود:** ينتج نسخة أحدث بدل خطأ تفرد، ولا يغير قواعد أي دورة أنشئت من نسخة أقدم.
...
- تصحيح تعريف الدورة أثناء جمع البيانات: الاسم وعدد الأوائل والفترات، ونسخة سياسة جديدة للقواعد، وحدث تدقيق `evaluation.cycle_updated`.
- قفل تواريخ الفترة بعد وصول مدخلات إليها، مع بقاء التسمية مسموحة.
- رفض إزالة مقرر له مدخلات، واستبعاد المرشح الذي فقد كل تسجيلاته عند إزالة مقرر بلا مدخلات.
- حفظ قالب قواعد وقراءته وإنشاء دورة منه، وإنتاج نسخة أحدث عند الحفظ بالاسم نفسه.
- إعادة قواعد محفوظة إلى شكل المحرر مع بقاء متغيرات المعايير من التعريف.
...
- `php artisan test`: **138 اختبارًا و1015 تحققًا**، بلا إخفاق.
- `FinalEvaluationSystemTest`: **30 اختبارًا و136 تحققًا**، بما فيها المصادر الآلية وعزل المعلم المحمول وحوارس تصحيح التعريف وقوالب القواعد.
...
1. تضبط القاعدة من محرر القواعد داخل معالج الدورة، أو تعدل بنية `config/evaluation.php` إذا كان التغيير في التعريف نفسه لا في قيمه.
2. تنشأ سياسة جديدة بدل تعديل سجل مستخدم في نتائج منشورة؛ هذا ما يفعله تعديل قواعد دورة قائمة تلقائيًا.
3. تربط الدورات الجديدة بالإصدار الجديد، ويختصر حفظ الإعداد كقالب هذه الخطوة للدورات اللاحقة.
