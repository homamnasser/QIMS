...
class TestDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Handy aliases — $users[0] is super-admin, [1-2] admin, [3-5] supervisor, [6-11] teacher
        ...
        $circlesRaw = [
            ['name'=>'حلقة الفاتحة',   'teacher_id'=>$users[6]->id,  'course_id'=>$courses[0]->id],
            ['name'=>'حلقة البقرة',    'teacher_id'=>$users[7]->id,  'course_id'=>$courses[0]->id],
            ['name'=>'حلقة آل عمران',  'teacher_id'=>$users[8]->id,  'course_id'=>$courses[1]->id],
            ['name'=>'حلقة النساء',    'teacher_id'=>$users[9]->id,  'course_id'=>$courses[2]->id],
            ['name'=>'حلقة المائدة',   'teacher_id'=>$users[10]->id, 'course_id'=>$courses[2]->id],
            ['name'=>'حلقة الأنعام',   'teacher_id'=>$users[11]->id, 'course_id'=>$courses[3]->id],
            ['name'=>'حلقة الأعراف',   'teacher_id'=>$users[6]->id,  'course_id'=>$courses[4]->id],
            ['name'=>'حلقة الأنفال',   'teacher_id'=>$users[7]->id,  'course_id'=>$courses[5]->id],
            ['name'=>'حلقة التوبة',    'teacher_id'=>$users[8]->id,  'course_id'=>$courses[6]->id],
            ['name'=>'حلقة يونس',      'teacher_id'=>$users[9]->id,  'course_id'=>$courses[7]->id],
            ['name'=>'حلقة هود',       'teacher_id'=>$users[10]->id, 'course_id'=>$courses[8]->id],
            ['name'=>'حلقة يوسف',      'teacher_id'=>$users[11]->id, 'course_id'=>$courses[9]->id],
        ];
        ...
        // ══════════════════════════════════════════════
        //  16. MEMORIZATIONS  (14 — unique student+page_number)
        // ══════════════════════════════════════════════
        ...
        $this->command->info('   ├── 24 تاريخ جلسة');
        $this->command->info('   ├── 12 حلقة');
        $this->command->info('   ├── 14 طالب');
        $this->command->info('   ├── 12 ملاحظة');
        $this->command->info('   ├── 12 سبر');
        $this->command->info('   ├── 14 تسميع');
        $this->command->info('   ├── 12 إنذار');
        $this->command->info('   ├── 14 امتحان');
        $this->command->info('   ├── 14 سجل حضور/غياب');
        $this->command->info('   └── 12 درجة طالب');
        $this->command->info('');
