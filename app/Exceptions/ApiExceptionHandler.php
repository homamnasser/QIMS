...
/**
 * يجمع كل قواعد تحويل الاستثناءات إلى استجابات JSON عربية موحدة، ويُسجَّل
 * من bootstrap/app.php عبر withExceptions().
 */
class ApiExceptionHandler
{
    ...
    public static function register(Exceptions $exceptions): void
    {
        ...
        $exceptions->renderable(function (UnauthorizedException $e, $request) {
            return self::apiError($request, 403, 'FORBIDDEN', 'ممنوع: لا تملك الصلاحيات المطلوبة للوصول.');
        });

        $exceptions->renderable(function (ProjectHasCoursesException $e, $request) {
        ...
        $exceptions->renderable(function (MosqueHasCoursesException $e, $request) {
            return self::apiError(
                $request,
                409,
                'MOSQUE_HAS_COURSES',
        ...
        $exceptions->renderable(function (MosqueHasScopedRecordsException $e, $request) {
            return self::apiError(
                $request,
                409,
        ...
        //
        // هذا المعالج يلتقط AuthenticationException قبل أن يصل إلى
        // unauthenticated() في معالج الإطار، لأن renderViaCallbacks تُنفَّذ أولاً.
        $exceptions->renderable(fn (Throwable $e, $request) => self::renderApiException($e, $request));
    }

    /**
     * يحوّل أي استثناء غير معالج إلى استجابة JSON موحدة على مسارات الـ API.
     */
    private static function renderApiException(Throwable $e, $request): ?JsonResponse
    {
        // استثناء يحمل استجابة جاهزة (مثل abort_if داخل middleware) يمر كما هو.
        if ($e instanceof HttpResponseException || ! self::isApiRequest($request)) {
            return null;
        }
        ...
        if ($e instanceof AuthenticationException) {
            return self::apiError($request, 401, 'UNAUTHENTICATED', 'غير مصادق — يجب تسجيل الدخول أولاً.');
        }
        ...
        if ($e instanceof HttpExceptionInterface) {
            ...
            return self::apiError($request, $status, $code, self::preferArabic($e->getMessage(), $message));
        }

        // خطأ قاعدة بيانات أو خطأ برمجي غير متوقع: يسجَّل كاملاً ولا يخرج منه شيء.
        $isDatabaseFailure = $e instanceof QueryException;
        self::reportUnexpected($e, $request, $isDatabaseFailure);
     ...
     */
    private static function apiError(
        $request,
        int $status,
        string $errorCode,
        string $message,
     ...
     */
    private static function reportUnexpected(Throwable $e, $request, bool $isDatabaseFailure): void
    {
        $context = [
     ...
     */
    private static function preferArabic(?string $thrown, string $fallback): string
    {
